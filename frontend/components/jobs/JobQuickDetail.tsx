...
export default function JobQuickDetail({ job }: JobQuickDetailProps) {
  if (!job) {
    return (
      <section className="w-full min-w-0 rounded-lg border border-slate-200 bg-white p-4 shadow-card">
        <div className="flex items-center gap-2.5">
          <div className="grid h-10 w-10 place-items-center rounded-md border border-slate-200 bg-slate-50 text-slate-400">
        ...
        <div className="mt-4 rounded-md border border-dashed border-slate-200 bg-slate-50/50 p-3 text-center">
  ...
  return (
    <section className="w-full min-w-0 overflow-hidden rounded-lg border border-rose-200 bg-white shadow-card">
      <div className="h-1 bg-[#b51d1a]" />
      ...
      <div className="p-4 sm:p-5">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-2">
          {[
            ...
            <div key={item.label} className="flex min-h-0 items-center gap-2 rounded-md border border-slate-100 bg-slate-50 px-3 py-2.5">
        ...
        <div className="mt-3 flex min-w-0 flex-wrap gap-1.5">
          {skillNames.length > 0 ? (
            skillNames.slice(0, 6).map((name) => (
              <span
                key={`${job.id}-${name}`}
                className="max-w-full min-w-0 shrink-0 truncate rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-600"
                title={name}
              >
                {name}
              </span>
            ...
            <span className="max-w-full min-w-0 break-words rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] text-slate-500">
        ...
        <div className="mt-4 grid min-h-0 grid-cols-2 gap-2">
          <Link
            href={`/jobs/${job.id}`}
            className="flex min-h-0 items-center justify-center gap-1.5 rounded-md bg-[#b51d1a] px-3 py-2.5 text-center text-[12px] font-bold text-white shadow-sm transition hover:bg-[#8f1515]"
          >
          ...
          <Link
            ...
            className="flex min-h-0 items-center justify-center gap-1.5 rounded-md border border-slate-300 bg-white px-3 py-2.5 text-center text-[12px] font-semibold text-slate-700 shadow-sm transition hover:border-slate-400 hover:bg-slate-50"
          >
