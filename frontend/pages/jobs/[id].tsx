...
export default function JobDetailPage() {
  ...
  async function handleResumeFileChange(file: File | null) {
    if (!file) return;
    if (!can("/api/v1/files", "POST")) {
      setApplySuccess("");
      setApplyError("Tài khoản hiện tại chưa có quyền tải CV lên.");
      return;
    }
  ...
  return (
    <>
      ...
      <main className="mx-auto min-h-screen max-w-[1180px] px-3 py-5 sm:px-4 sm:py-6">
      <header className="overflow-hidden rounded-lg border border-slate-200 bg-white p-5 text-slate-950 shadow-soft sm:p-6 lg:p-7">

        <div className="mb-4 flex flex-wrap items-center gap-2">
          <Link
            href="/"
            className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-white px-3.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50"
          >
          ...
          <button
            type="button"
            onClick={copyJobLink}
            className="inline-flex items-center gap-1.5 rounded-md border border-rose-200 bg-rose-50 px-3.5 py-1.5 text-xs font-semibold text-[#b51d1a] transition hover:bg-rose-100"
          >
            {linkCopied ? (
              <>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-start">
          <div className="min-w-0">
            {/* Eyebrow */}
            <div className="mb-2 inline-flex items-center gap-2 rounded-md border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-semibold text-[#b51d1a]">
              <span className="h-1.5 w-1.5 rounded-full bg-[#b51d1a] animate-pulse" />
              Đang tuyển dụng
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Chi tiết tuyển dụng</p>
                <h1 className="mt-1.5 text-2xl font-extrabold leading-tight text-slate-950 sm:text-3xl lg:text-4xl">{job.name}</h1>
                <p className="mt-2 flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-700">
                  {companyName}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {heroMetaItems.map((item) => (
                    <span
                      key={`${job.id}-${item}`}
                      className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-700"
                    >
                      {item}
                    </span>
                  ))}
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {skillNames.slice(0, 8).map((name) => (
                      <span
                        ...
                        className="inline-flex items-center gap-1 rounded-md border border-rose-200 bg-rose-50 px-2.5 py-1 text-[11px] font-semibold text-[#b51d1a]"
                      >
                        {name}
                      </span>
                    ))}
          ...
          <div className="grid gap-2.5 rounded-lg border border-rose-200 bg-rose-50 p-5">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-[#b51d1a]">Mức lương</p>
              <p className="mt-2 break-words text-[28px] font-extrabold leading-none text-slate-950">
                {formatCurrencyVnd(job.salary)}
              </p>
              <p className="mt-0.5 text-xs text-slate-500">/ tháng</p>
            </div>
            <div className="grid gap-2">
              <div className="flex items-center gap-2 rounded-md border border-rose-200 bg-white px-3.5 py-2.5">
                <svg className="h-4 w-4 shrink-0 text-[#b51d1a]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <span className="text-sm font-semibold text-slate-800">{formatLocationLabel(job.location)}</span>
              </div>
              <div className="flex items-center gap-2 rounded-md border border-rose-200 bg-white px-3.5 py-2.5">
                <svg className="h-4 w-4 shrink-0 text-[#b51d1a]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="text-sm font-semibold text-slate-800">Còn {formatDateVi(job.endDate)}</span>
              </div>
              <div className="flex items-center gap-2 rounded-md border border-rose-200 bg-white px-3.5 py-2.5">
                <svg className="h-4 w-4 shrink-0 text-[#b51d1a]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-1a3 3 0 00-5.196V6a3 3 0 00-5.196V4h5" />
                </svg>
                <span className="text-sm font-semibold text-slate-800">{job.quantity} vị trí</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.7fr)_360px] xl:gap-5">
        ...
        <aside className="grid content-start gap-3 xl:sticky xl:top-24">
          ...
          <section className="rounded-lg border border-rose-200 bg-white p-4 shadow-soft">
            ...
            <div className="mt-4 grid gap-2.5">
              <Link
                ...
                className="flex items-center justify-center gap-2 rounded-md border border-rose-200 bg-rose-50 px-3 py-2.5 text-center text-sm font-semibold text-rose-600 transition hover:border-rose-400 hover:bg-rose-100"
              >
              ...
              {authStatus !== "authenticated" ? (
                <div className="grid gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-xs text-amber-800">
                ...
                >
                  ...
                  {can("/api/v1/files", "POST") ? (
                    <label className="grid gap-1.5 text-xs text-slate-600">
                      ...
                      <input
                        ...
                        onChange={(event) => void handleResumeFileChange(event.target.files?.[0] ?? null)}
                        className="rounded-lg border border-dashed border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 file:mr-3 file:rounded-md file:border-0 file:bg-slate-900 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-white disabled:cursor-not-allowed disabled:opacity-60"
                      />
                      {uploadingCv ? <span className="text-xs font-semibold text-rose-600">Đang tải CV lên...</span> : null}
                    </label>
                  ) : null}
                  <label className="grid gap-1.5 text-xs text-slate-600">
                    URL CV của bạn
                  ...
                  <button
                    type="submit"
                    disabled={applying || uploadingCv || cvUrl.trim().length === 0}
                    className="rounded-md bg-emerald-600 px-3 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
                  >
              ...
              <Link
                href="/"
                className="rounded-md border border-slate-300 bg-white px-3 py-2.5 text-center text-sm font-semibold text-slate-700 hover:bg-slate-100"
              >
                Quay lại danh sách
              </Link>
            </div>
