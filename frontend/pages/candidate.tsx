...
export default function CandidateWorkspacePage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (savedJobData) {
        ...
        saveBookmarks(savedIds, bookmarkScope);
        setSavedJobs(savedJobData);
      } else {
        const bookmarkIds = getBookmarks(bookmarkScope);
        setSavedJobs(jobData.filter((j) => bookmarkIds.includes(j.id)));
      }
    } catch (loadError) {
  ...
  }, [status, isAllowed, bookmarkScope]);
      ...
      <DashboardHero
        ...
        actions={
          <>
            ...
            <Link
              href="/account"
              className="rounded-md border border-white/30 bg-white/10 px-4 py-2 text-sm font-semibold text-slate-100 hover:bg-white/20"
            >
              Cập nhật tài khoản
            </Link>
          </>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft sm:p-6">
            ...
            <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
              {savedJobs.slice(0, 6).map((job) => (
                <article key={job.id} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                  <div className="flex items-start gap-2.5">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        void unsaveJobWithAuth(job.id).catch(() => undefined);
                      }}
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    <Link
                      href={`/jobs/${job.id}`}
                      className="flex-1 rounded-md bg-rose-600 px-2 py-1.5 text-center text-xs font-bold text-white transition hover:bg-rose-700"
                    >
                      Ứng tuyển
                    </Link>
                    <Link
                      href={`/jobs/${job.id}`}
                      className="flex-1 rounded-md border border-slate-300 px-2 py-1.5 text-center text-xs font-semibold text-slate-700 transition hover:bg-slate-50"
                    >
                      Chi tiết
                    </Link>
                  </div>
                  ...
                  <div className="mt-2 grid gap-1 text-xs text-slate-500">
                    ...
                    {item.url ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex w-fit rounded-md border border-slate-300 bg-white px-2 py-1 font-semibold text-slate-700 hover:bg-slate-100"
                      >
                        Xem CV đã gửi
                      </a>
                    ) : null}
        ...
        <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft sm:p-6">
          ...
          <div className="mt-4 grid gap-2.5">
            {suggestedJobs.map((job) => (
              <article key={job.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3.5">
          ...
          <Link
            href="/"
            className="mt-4 inline-flex rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100"
          >
